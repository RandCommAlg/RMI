...
{* 
-- SOME INTERNAL TESTING CODE -- NOT FOR SHARING --  
...
beta1mtx = matrix(beta1)
beta1shape = new BettiTally from mat2betti  matrix pack(1+pdim(R^1/I), apply(flatten entries beta1mtx, i-> if i>0 then i=1 else i=0))

select(flatten entries beta1shape,i->i>0)
scan(entries beta1shape,i-> i>0)
viewHelp select
beta1shapeTable = new MutableHashTable from beta1shape

positive = i->if i>0 then true else false

select(beta1shapeTable, positive)
select(beta1shape,positive)
scan ((values beta1shapeTable)#2, i-> if positive(i) then print i)

beta1shapeOnly = new BettiTally from mat2betti beta1shape
*} 
