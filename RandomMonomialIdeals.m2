...
doc ///
 ...
 Description
  ...
  Text 
   ...
   This model was inspired by random graphs. To parallel the two variants of the Erdos-Renyi model for graphs - 
   fixing either the probability of an edge or the total number of edges -
   the package also includes the model that generated a fixed  {\em number} of monomials: 
  Example
    n=3; D=2; M=3; N=4;
  ...
  Text
   Once a sample is generated, one can compute various statistics of algebraic properties of the sample. 
  ...
  Text
   ...
   defines two new types of object, @TO Model@ and @TO Sample@, which allow for convenient storage of statistics from a sample of algebraic objects and streamlines writing sample data into files.
  ...
  Text
   The output is a hash table with 4 entries. To obtain the random sets of monomials (the actual data we are interested in),
  ...
  Example
   ...
   myData_0
  Text 
   We can also use the object of type @TO Sample@ to calculate the mean, standard deviation, and tally of the dimension of the ideals generated by the sample: 
  Example
...
doc ///
 Key
  (sample,String)
 Headline
  creates a sample from a folder on disk
 Usage
  sample(String)
 Inputs
  FileName: String
