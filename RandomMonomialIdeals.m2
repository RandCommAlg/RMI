...
doc ///
 ...
 Description
  ...
  Text 
   For formal definition of the model, see Section 1 of @HREF"https://arxiv.org/abs/1701.07130"@. 
   To parallel random graphs with two variants of the model that fix either probability of an edge or total number of edges,
  ...
  Text
   ...
   For example, below we create a sample of size 10 over the Erdos-Renyi distribution on monomials over the ring $Q[x,y]$ with $D=4$, and $p=0.5$: 
  Example 
   print("TO DO: need to uncomment sample: it's the following line in example")
   -- UNCOMMENT ONCE SAMPLE IS MERGED WITH MASTER! 
   --sample(ER(QQ[x,y],4,0.5),10)
  Text 
   Create a sample of size 15 over the graded Erdos-Renyi distribution on monomials over the ring $C[z_1..z_8]$ with $D=2$, and $p={0.25,0.5}$: 
  Example
   print("TO DO: need to uncomment sample: it's the following line in example")
   --sample(ER(CC[z_1..z_8],2,{0.25,0.75}),15)
  Text 
   To use the sample object to calculate the mean, standard deviation, and tally of the dimension of the ideals generated by the sample: 
  Example
   print("TO DO: need to uncomment statistics: it's the following line in example")
 ...
 SeeAlso
  randomMonomialSet
  Verbose
  --Sample  -- **** TO DO: uncomment this line too! **** 
///
