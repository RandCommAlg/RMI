...
doc ///
 ...
 Description
  ...
  Text
   The package also allows for sampling from the {\em graded} version of the fixed number of monomials model, where we prespecify the requested number of monomials of each degree. 
  ...
  Text
   Finally we can request the monomial sets generated by the {\em graded} model with fixed number of monomials, to be minimal generating sets. 
  Example
   n=3; D=3; N=4;
  ...
  Text
   ...
   For example, below we create a sample of size 10 over the Erdos-Renyi distribution $\mathcal B(n, D, p)$ on monomials over the ring $Q[x,y]$ with $D=4$, and $p=0.5$, and 
  ...
  Example
   sample1 = sample(ER(QQ[x,y],4,0.5),10)
...
doc ///
 ...
 Description
  ...
  Text
   Observe that there are at most one degree-1 monomial, one degree-2 monomial, and one degree-3 monomial.
   
   If Strategy=>"ER", the default setting for the graded fixed number of generators version of the method, each set of monomials used to generate a @TO MonomialIdeal@ in our list is not necessarily minimal.
   Else if Strategy=> "Minimal" then each monomial ideal in the list is generated by minimal sets of $M_d$ monomials, or maximum number possible,  of total degree $d$, starting from the smallest degree.
